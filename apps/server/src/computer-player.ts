...
export class StockfishComputer implements ComputerMoveProvider {
  ...
  async chooseMove(fen: string, level: ComputerLevel): Promise<ComputerMove> {
    ...
    try {
      return await this.runtime.run(async (engine) => {
  ...
  private async bestMove(engine: StockfishEngine, fen: string, thinkTime: number): Promise<string> {
    ...
    const line = await this.runtime.sendAndWait(engine, `go movetime ${thinkTime}`, (output) => output.startsWith("bestmove "));
